...
void TaskBLE(void const * argument);
void TaskACC(void const * argument);
...
int main(void)
{
  ...
  osThreadStaticDef(TASK_BLE, TaskBLE, osPriorityHigh, 0, 512, TASK_BLEBuffer, &TASK_BLEControlBlock);
  TASK_BLEHandle = osThreadCreate(osThread(TASK_BLE), NULL);

  /* definition and creation of TASK_ACC */
  osThreadStaticDef(TASK_ACC, TaskACC, osPriorityNormal, 0, 512, TASK_ACCBuffer, &TASK_ACCControlBlock);
